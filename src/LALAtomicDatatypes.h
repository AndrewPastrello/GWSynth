...
typedef char CHAR; /**< One-byte signed integer, see \ref LALDatatypes for more
        details */
typedef unsigned char UCHAR;   /**< One-byte unsigned integer, see \ref
                                LALDatatypes for more details */
typedef unsigned char BOOLEAN; /**< Boolean logical type, see \ref LALDatatypes
                                for more details */
...
typedef uint64_t UINT8; /**< Eight-byte unsigned integer; on some platforms this
                         is equivalent to <tt>unsigned long int</tt> instead. */
#endif

/** MACRO to initialize arbitrary variable 'x' to zero */
#define XLAL_INIT_MEM(x) memset(&(x), 0, sizeof((x)))
/** C99 MACRO to declare and zero-initialize a variable, use as "type
   XLAL_INIT_DECL(var);".
    To declare and zero-initialize an array variable, use as "type
   XLAL_INIT_DECL(var, [n]);" */
...
/* Real types */
typedef float
    REAL4; /**< Single precision real floating-point number (4 bytes). */
typedef double
    REAL8; /**< Double precision real floating-point number (8 bytes). */
...
#endif
typedef float _Complex COMPLEX8;   /**< Single-precision floating-point complex
                                   number (8 bytes total) */
typedef double _Complex COMPLEX16; /**< Double-precision floating-point complex
                                   number (16 bytes total) */

#define crectf(re, im) \
  (((REAL4)(re)) +     \
   I * ((REAL4)(       \
           im))) /**< Construct a COMPLEX8 from real and imaginary parts */
#define crect(re, im) \
  (((REAL8)(re)) +    \
   I * ((REAL8)(      \
           im))) /**< Construct a COMPLEX16 from real and imaginary parts */
#define cpolarf(r, th) \
  (((REAL4)(r)) *      \
   cexpf(crectf(       \
       0, th))) /**< Construct a COMPLEX8 from polar modulus and argument */
#define cpolar(r, th) \
  (((REAL8)(r)) *     \
   cexp(crect(        \
       0, th))) /**< Construct a COMPLEX16 from polar modulus and argument */
// #endif
// /** \cond DONT_DOXYGEN */
