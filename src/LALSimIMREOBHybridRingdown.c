 ...
 * This method is currently used for EOBNRv2 and SEOBNRv1 models. The only
 * difference between the two models in ring-down waveform is the pseudo-QNM
 * introduced in the latter (see Taracchini et al. PRD 86, 024011 (2012) for
 * more details).
 */
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Defining matrices to store the coefficients that enter amplitude and phase of
 * the RD signal of SEOBNRv4 and SEOBNRv4HM */

static REAL8 XLALCalculateRDAmplitudeCoefficient1(UINT4 modeL, UINT4 modeM,
                                                  REAL8 eta, REAL8 chi) {
  ...
  return A1coeff00[modeL][modeM] + A1coeff01[modeL][modeM] * chi +
         A1coeff02[modeL][modeM] * chi * chi + A1coeff10[modeL][modeM] * eta +
         A1coeff11[modeL][modeM] * eta * chi +
         A1coeff20[modeL][modeM] * eta * eta +
         A1coeff21[modeL][modeM] * eta * eta * chi +
         A1coeff31[modeL][modeM] * eta * eta * eta * chi;
}

static REAL8 XLALCalculateRDAmplitudeCoefficient2(UINT4 modeL, UINT4 modeM,
                                                  REAL8 eta, REAL8 chi) {
  ...
  return A2coeff00[modeL][modeM] + A2coeff01[modeL][modeM] * chi +
         A2coeff10[modeL][modeM] * eta + A2coeff11[modeL][modeM] * eta * chi +
         A2coeff20[modeL][modeM] * eta * eta +
         A2coeff21[modeL][modeM] * eta * eta * chi +
         A2coeff31[modeL][modeM] * eta * eta * eta * chi;
}

static REAL8 XLALCalculateRDPhaseCoefficient1(UINT4 modeL, UINT4 modeM,
                                              REAL8 eta, REAL8 chi) {
  ...
  return P1coeff00[modeL][modeM] + P1coeff01[modeL][modeM] * chi +
         P1coeff02[modeL][modeM] * chi * chi + P1coeff10[modeL][modeM] * eta +
         P1coeff11[modeL][modeM] * eta * chi +
         P1coeff20[modeL][modeM] * eta * eta +
         P1coeff21[modeL][modeM] * eta * eta * chi +
         P1coeff31[modeL][modeM] * eta * eta * eta * chi;
}

static REAL8 XLALCalculateRDPhaseCoefficient2(UINT4 modeL, UINT4 modeM,
                                              REAL8 eta, REAL8 chi) {
  ...
  return P2coeff00[modeL][modeM] + P2coeff01[modeL][modeM] * chi +
         P2coeff02[modeL][modeM] * chi * chi +
         P2coeff12[modeL][modeM] * chi * chi * eta +
         P2coeff10[modeL][modeM] * eta + P2coeff11[modeL][modeM] * eta * chi +
         P2coeff20[modeL][modeM] * eta * eta +
         P2coeff21[modeL][modeM] * eta * eta * chi +
         P2coeff31[modeL][modeM] * eta * eta * eta * chi;
}

static INT4 XLALSimFindIndexMaxAmpli(UINT4 *indAmax, REAL8Vector *timeVec,
                                     REAL8Vector *ampWave, REAL8 *valAmax,
                                     REAL8 tofAmax);
 ...
 * y is a 16-d vector depending on inspiral-plunge waveforms and their
 * derivatives near merger.
 */
static INT4 XLALSimIMREOBHybridRingdownWave(
    ...
    REAL8 *matchrange /**<< Times which determine the comb of ringdown
                               attachment */
) {
  /* XLAL error handling */
  ...
  t5 = (matchrange[0] - matchrange[1]) * m;
  rt = -t5 / 5.;
   ...
   */
  /* The half of A defined here corresponds to matrices M1 and -M2 in the DCC
   * document T1100433 */
  /* Define y here as "hderivs" */
  for (i = 0; i < nmodes; ++i) {
    gsl_matrix_set(coef, 0, i, 1);
    gsl_matrix_set(coef, 1, i, -cimag(modefreqs->data[i]));
    gsl_matrix_set(coef, 2, i,
                   exp(-cimag(modefreqs->data[i]) * t1) *
                       cos(creal(modefreqs->data[i]) * t1));
    gsl_matrix_set(coef, 3, i,
                   exp(-cimag(modefreqs->data[i]) * t2) *
                       cos(creal(modefreqs->data[i]) * t2));
    gsl_matrix_set(coef, 4, i,
                   exp(-cimag(modefreqs->data[i]) * t3) *
                       cos(creal(modefreqs->data[i]) * t3));
    gsl_matrix_set(coef, 5, i,
                   exp(-cimag(modefreqs->data[i]) * t4) *
                       cos(creal(modefreqs->data[i]) * t4));
    gsl_matrix_set(coef, 6, i,
                   exp(-cimag(modefreqs->data[i]) * t5) *
                       cos(creal(modefreqs->data[i]) * t5));
    gsl_matrix_set(
        coef, 7, i,
        exp(-cimag(modefreqs->data[i]) * t5) *
    ...
    gsl_matrix_set(coef, 10, i,
                   -exp(-cimag(modefreqs->data[i]) * t1) *
                       sin(creal(modefreqs->data[i]) * t1));
    gsl_matrix_set(coef, 11, i,
                   -exp(-cimag(modefreqs->data[i]) * t2) *
                       sin(creal(modefreqs->data[i]) * t2));
    gsl_matrix_set(coef, 12, i,
                   -exp(-cimag(modefreqs->data[i]) * t3) *
                       sin(creal(modefreqs->data[i]) * t3));
    gsl_matrix_set(coef, 13, i,
                   -exp(-cimag(modefreqs->data[i]) * t4) *
                       sin(creal(modefreqs->data[i]) * t4));
    gsl_matrix_set(coef, 14, i,
                   -exp(-cimag(modefreqs->data[i]) * t5) *
                       sin(creal(modefreqs->data[i]) * t5));
    gsl_matrix_set(
        coef, 15, i,
        exp(-cimag(modefreqs->data[i]) * t5) *
  ...
  for (i = 0; i < 2; ++i) {
    gsl_vector_set(hderivs, i,
                   inspwave1->data[(i + 1) * inspwave1->vectorLength - 1]);
    gsl_vector_set(hderivs, i + nmodes,
                   inspwave2->data[(i + 1) * inspwave2->vectorLength - 1]);
    gsl_vector_set(hderivs, i + 6,
                   inspwave1->data[i * inspwave1->vectorLength]);
    gsl_vector_set(hderivs, i + 6 + nmodes,
                   inspwave2->data[i * inspwave2->vectorLength]);
  }
  ...
  REAL8 timeOffset = fmod(matchrange[1], dt / m) * dt;
...
static INT4 XLALGenerateHybridWaveDerivatives(
    ...
    REAL8 *matchrange,    /**<< Times which determine the size of the comb */
  ...
  rt = (matchrange[1] - matchrange[0]) / 5.;
  tlist[0] = matchrange[0];
  tlist[1] = tlist[0] + rt;
  tlist[2] = tlist[1] + rt;
  tlist[3] = tlist[2] + rt;
  tlist[4] = tlist[3] + rt;
  tlist[5] = matchrange[1];

  /* Set the length of the interpolation vectors */
  vecLength = (m * matchrange[2] / dt) + 1;

  /* Getting interpolation and derivatives of the waveform using gsl spline
   * routine */
 ...
 * STEP 1) Get mass and spin of the final black hole and the complex ringdown
 * frequencies STEP 2) Based on least-damped-mode decay time, allocate memory
 * for rigndown waveform STEP 3) Get values and derivatives of inspiral
 * waveforms at matching comb points STEP 4) Solve QNM coefficients and generate
 * ringdown waveforms STEP 5) Stitch inspiral and ringdown waveoforms
 */
...
static UNUSED INT4 XLALSimIMREOBHybridAttachRingdown(
    ...
    const REAL8 mass1,    /**<< First component mass (in Solar masses) */
    const REAL8 mass2,    /**<< Second component mass (in Solar masses) */
    const REAL8 spin1x, /**<<The spin of the first object; only needed for spin
                           waveforms */
    const REAL8 spin1y, /**<<The spin of the first object; only needed for spin
                           waveforms */
    const REAL8 spin1z, /**<<The spin of the first object; only needed for spin
                           waveforms */
    const REAL8 spin2x, /**<<The spin of the second object; only needed for spin
                           waveforms */
    const REAL8 spin2y, /**<<The spin of the second object; only needed for spin
                           waveforms */
    const REAL8 spin2z, /**<<The spin of the second object; only needed for spin
                           waveforms */
  ...
  if (XLALSimIMREOBGenerateQNMFreqV2(modefreqs, mass1, mass2, spin1, spin2, l,
                                     m, nmodes, approximant) == XLAL_FAILURE) {
    XLALDestroyCOMPLEX16Vector(modefreqs);
    XLAL_ERROR(XLAL_EFUNC);
  }
  // UINT4 i;
  // for (i=0; i<nmodes; i++){
  //    printf("Stas mode %d: %f + i %f \n", i, creal(modefreqs->data[i]),
  //    cimag(modefreqs->data[i]));
  //}

  /* Call XLALSimIMREOBFinalMassSpin() to get mass and spin of the final black
   * hole */
  if (XLALSimIMREOBFinalMassSpin(&finalMass, &finalSpin, mass1, mass2, spin1,
                                 spin2, approximant) == XLAL_FAILURE) {
    XLAL_ERROR(XLAL_EFUNC);
  }
  ...
  if (approximant == SEOBNRv1) {
    ...
    a = (spin1[2] + spin2[2]) / 2. * (1.0 - 2.0 * eta) +
        (spin1[2] - spin2[2]) / 2. * (mass1 - mass2) / (mass1 + mass2);
    NRPeakOmega22 = XLALSimIMREOBGetNRSpinPeakOmega(l, m, eta, a) / mTot;
    /*printf("a and NRomega in QNM freq: %.16e %.16e %.16e %.16e
     * %.16e\n",spin1[2],spin2[2], mTot/LAL_MTSUN_SI,a,NRPeakOmega22*mTot); */
    modefreqs->data[7] =
        (NRPeakOmega22 / finalMass + creal(modefreqs->data[0])) / 2.;
    modefreqs->data[7] += I * 10. / 3. * cimag(modefreqs->data[0]);
  }

  if (approximant == SEOBNRv2)  // See pages 6 to 12 of the dcc document
                                // T1400476-v3 for expressions in this block.
  {
    /* Replace the last two QNMs with pQNMs */
    /* We assume aligned/antialigned spins here */
    /* Definitions of a, chi and NRPeakOmega22, where the last one is an
     * approximation of \phi'[tmatch] in T1400476-v3. */
    a = (spin1[2] + spin2[2]) / 2. * (1.0 - 2.0 * eta) +
        (spin1[2] - spin2[2]) / 2. * (mass1 - mass2) / (mass1 + mass2);
    NRPeakOmega22 = XLALSimIMREOBGetNRSpinPeakOmegav2(l, m, eta, a) / mTot;

    /* Define chi */
    chi = (spin1[2] + spin2[2]) / 2. + (spin1[2] - spin2[2]) / 2. *
    ...
    /*printf("a, chi and NRomega in QNM freq: %.16e %.16e %.16e %.16e %.16e
     * %.16e\n",
     ...
     */
    modefreqs->data[7] = (2. / 3. * NRPeakOmega22 / finalMass) +
                         (1. / 3. * creal(modefreqs->data[0]));
    modefreqs->data[7] += I * 3.5 / 0.9 * cimag(modefreqs->data[0]);
    modefreqs->data[6] = (3. / 4. * NRPeakOmega22 / finalMass) +
                         (1. / 4. * creal(modefreqs->data[0]));
    ...
    if ((eta > 30. / 31. / 31. && eta <= 10. / 121. && chi >= 0.8) ||
        (eta <= 30. / 31. / 31. && chi >= 0.8 &&
         chi < 0.9)) {  // This is case 4 in T1400476-v3
      sh = -9. * (eta - 0.25) *
           (1. + 2. * exp(-(chi - 0.85) * (chi - 0.85) / 0.05 / 0.05)) *
           (1. + 1. / (1. + exp((eta - 0.01) / 0.001)));
      kk = 0.7 + 0.3 * exp(100. * (eta - 0.25));
      kt1 = 0.5 * sqrt(1. + 800.0 * eta * eta / 3.0) - 0.125;
      kt2 = 0.5 * pow(1. + 0.5 * eta * sqrt(eta) / 0.0225, 2. / 3.) - 0.2;
      modefreqs->data[4] = 0.4 * (1. + kk) * creal(modefreqs->data[6]) +
                           I * cimag(modefreqs->data[6]) /
                               (2.5 * kt2 * exp(-(eta - 0.005) / 0.03));
      modefreqs->data[5] = 0.4 * (1. + kk) * creal(modefreqs->data[7]) +
                           I * cimag(modefreqs->data[7]) /
                               (1.5 * kt1 * exp(-(eta - 0.005) / 0.03));
      modefreqs->data[6] =
          kk * creal(modefreqs->data[6]) + I * cimag(modefreqs->data[6]) / kt2;
      modefreqs->data[7] =
          kk * creal(modefreqs->data[7]) + I * cimag(modefreqs->data[7]) / kt1;
    }
    if (eta < 30. / 31. / 31. && chi >= 0.9) {  // This is case 5 in T1400476-v3
      ...
      modefreqs->data[4] = 1.1 * 0.4 * (1. + kk) * creal(modefreqs->data[6]) +
                           I * cimag(modefreqs->data[6]) /
                               (1.05 * 2.5 * kt2 * exp(-(eta - 0.005) / 0.03));
      modefreqs->data[5] = 0.4 * (1. + kk) * creal(modefreqs->data[7]) +
                           I * cimag(modefreqs->data[7]) /
                               (1.05 * 1.5 * kt1 * exp(-(eta - 0.005) / 0.03));
      modefreqs->data[6] =
          kk * creal(modefreqs->data[6]) + I * cimag(modefreqs->data[6]) / kt2;
      modefreqs->data[7] =
          kk * creal(modefreqs->data[7]) + I * cimag(modefreqs->data[7]) / kt1;
    ...
    if (eta > 10. / 121. && chi >= 0.8) {  // This is case 6 in T1400476-v3
      ...
      kt2 = 0.5 * pow(1. + 0.5 * eta * sqrt(eta) / 0.0225, 2. / 3.) - 0.2;
      modefreqs->data[6] = kk * creal(modefreqs->data[6]) +
                           I * cimag(modefreqs->data[6]) / 0.95 / kt2;
      modefreqs->data[7] =
          kk * creal(modefreqs->data[7]) + I * cimag(modefreqs->data[7]) / kt1;
    }
  ...
  // Move ringdown comb boundaries to sampling points to avoid numerical
  // artifacts.
  matchrange[0] -= fmod(matchrange[0], dt / mTot);
  matchrange[1] -= fmod(matchrange[1], dt / mTot);
  /*for (j = 0; j < nmodes; j++)
   * {
   * printf("QNM frequencies: %d %d %d %f
  ...
  if (matchrange[0] * mTot / dt < 5 ||
      matchrange[1] * mTot / dt > matchrange[2] * mTot / dt - 2) {
    XLALPrintError("More inspiral points needed for ringdown matching.\n");
    printf("%.16e,%.16e,%.16e\n", matchrange[0] * mTot / dt,
           matchrange[1] * mTot / dt, matchrange[2] * mTot / dt - 2);
    XLALDestroyCOMPLEX16Vector(modefreqs);
  ...
  /*
   * STEP 2) Based on least-damped-mode decay time, allocate memory for rigndown
   * waveform
   */

  /* Create memory for the ring-down and full waveforms, and derivatives of
   * inspirals */
  ...
  if (!rdwave1 || !rdwave2 || !rinspwave || !dinspwave || !ddinspwave ||
  ...
  /*
   * STEP 3) Get values and derivatives of inspiral waveforms at matching comb
   * points
   */

  /* Generate derivatives of the last part of inspiral waves */
  /* Get derivatives of signal1 */
  if (XLALGenerateHybridWaveDerivatives(rinspwave, dinspwave, ddinspwave,
                                        timeVec, signal1, matchrange, dt, mass1,
                                        mass2) == XLAL_FAILURE) {
    XLALDestroyCOMPLEX16Vector(modefreqs);
  ...
  if (XLALGenerateHybridWaveDerivatives(rinspwave, dinspwave, ddinspwave,
                                        timeVec, signal2, matchrange, dt, mass1,
                                        mass2) == XLAL_FAILURE) {
    XLALDestroyCOMPLEX16Vector(modefreqs);
    XLALDestroyREAL8Vector(rdwave1);
    XLALDestroyREAL8Vector(rdwave2);
  ...
  if (XLALSimIMREOBHybridRingdownWave(rdwave1, rdwave2, dt, mass1, mass2,
                                      inspwaves1, inspwaves2, modefreqs,
                                      matchrange) == XLAL_FAILURE) {
    XLALDestroyCOMPLEX16Vector(modefreqs);
    XLALDestroyREAL8Vector(rdwave1);
    XLALDestroyREAL8Vector(rdwave2);
    XLALDestroyREAL8Vector(rinspwave);
  ...
  // UINT4 attachIdx = matchrange[1] * mTot / dt;
  UINT4 attachIdx = round(matchrange[1] * mTot / dt);
  for (j = 1; j < Nrdwave; ++j) {
    signal1->data[j + attachIdx] = rdwave1->data[j];
    signal2->data[j + attachIdx] = rdwave2->data[j];
  }

  memset(signal1->data + Nrdwave + attachIdx, 0,
         (signal1->length - Nrdwave - attachIdx) * sizeof(REAL8));
  memset(signal2->data + Nrdwave + attachIdx, 0,
         (signal2->length - Nrdwave - attachIdx) * sizeof(REAL8));
...
// RC: this function is not actually looking for the max, but it's looking for
// the index at which t corresponds to tofAmax which is fed as an input. For the
// model SEOBNRv4 this tofAmax is the peak of the amplitude
// of the 22 mode. For the model SEOBNRv4HM tofAmax is the maximum of the
// amplitude of the 22 mode with the exception of the the 55 mode for which this
// function is finding the index of tpeak22 -10M, which I
// feed as input as tofAmax.
static INT4 XLALSimFindIndexMaxAmpli(UINT4 *indAmax, REAL8Vector *timeVec,
                                     REAL8Vector *ampWave, REAL8 *valAmax,
                                     REAL8 tofAmax) {
  if (indAmax == NULL || timeVec == NULL || ampWave == NULL ||
      valAmax == NULL) {
    return XLAL_FAILURE;
  }
  INT4 debugSB = 0;
...
/**
 * The main  function for performing the ringdown attachment for SEOBNRv4 (and
 * beyond) This is the function which gets called by the code generating the
 * full IMR waveform once generation of the inspiral part has been completed.
 * The ringdown is attached by factoring the less damped harmonics and apply
 * tanh fit to the rest. STEP 1) Get mass and spin of the final black hole and
 * the complex ringdown frequencies STEP 2) Apply the fit function from the
 * attachment time STEP 3) Construct the full RD by applying (factor) of less
 * damped 220 mode STEP 4) Constructing the RD stitched to the inspiral-merger
 * See Section 5.2 of https://dcc.ligo.org/T1600383
 */
static UNUSED INT4 XLALSimIMREOBAttachFitRingdown(
    REAL8Vector *signal1, /**<< OUTPUT, Real of inspiral waveform to which we
                             attach ringdown */
    REAL8Vector *signal2, /**<< OUTPUT, Imag of inspiral waveform to which we
                             attach ringdown */
    const INT4 l,         /**<< Current mode l */
    ...
    /**<< Time values chosen as points for performing comb matching */
    Approximant approximant, /**<<The waveform approximant being used */
    UINT4 *indAmpMax /**<<This is used only for SEOBNRv4HM, it is needed to
                      remember the attaching point of the 22 mode without
                      computing it every time */
) {
  if (mass1 <= 0. || mass2 <= 0. || fabs(spin1x) > 1. || fabs(spin1y) > 1. ||
      fabs(spin1z) > 1. || fabs(spin2x) > 1. || fabs(spin2y) > 1. ||
      fabs(spin2z) > 1. ||
  ...
  COMPLEX16Vector *modefreqs;
  COMPLEX16Vector
      *modefreqs22;  // RC: we use this variable to compute the damping time of
                     // the 22 mode which will be used to set the lenght of the
                     // ringdown for all the modes
  ...
  if (debugSB) {
    printf(
        "RDfit: we use spin1 %f, spin2 %f, and it should be dimensionless "
        "[-1,1] \n",
        chi1, chi2);
    printf("We use approximant = %d \n", appr);
  }
  REAL8 chi = 0.5 * (chi1 + chi2) + 0.5 * (chi1 - chi2) * (mass1 - mass2) /
                                        (mass1 + mass2) / (1.0 - 2.0 * eta);
  ...
  if (XLALSimIMREOBGenerateQNMFreqV2(modefreqs22, mass1, mass2, spin1, spin2, 2,
                                     2, 1, appr) == XLAL_FAILURE) {
    XLALDestroyCOMPLEX16Vector(modefreqs);
    XLAL_ERROR(XLAL_EFUNC);
  }
  /* Least-damped QNM */
  COMPLEX16 sigmalm0;
  sigmalm0 = (-cimag(modefreqs->data[0]) - I * creal(modefreqs->data[0])) *
             (mtot * LAL_MTSUN_SI);
  ...
  if (debugSB) {
    ...
    printf("matchpoints are: %.16f,  %.16f,   %.16f, %.16f\n", matchrange[0],
           matchrange[1], matchrange[2], matchrange[3]);
  ...
  for (i = 0; i < timeVec->length; i++) {
    ampWave->data[i] = sqrt(signal1->data[i] * signal1->data[i] +
                            signal2->data[i] * signal2->data[i]);
    now = atan2(signal2->data[i], signal1->data[i]);
    if (i > 0) {
      /* Unwrapping */
      corph = now - prev;
      corph = corph > LAL_PI ? corph - LAL_TWOPI
                             : (corph < -LAL_PI ? corph + LAL_TWOPI : corph);
  ...
  if (debugSB) {
    ...
    for (i = 0; i < timeVec->length; i++) {
      fprintf(fout, "%f   %.16e   %.16e   %.16e   %.16e  \n", timeVec->data[i],
              ampWave->data[i], phWave->data[i], signal1->data[i],
              signal2->data[i]);
    }

    fclose(fout);
  }
  /* For the modes 22, 33, 21, 44 search for index at which the maximum of the
   * amplitude of the 22 mode occurs */
  /* For the mode 55 search for the index corresponding at the time tpeak22-10M,
   * where tpeak22 is the time of the maximum of the amplitude of the 22 mode*/
  /* See Eq.(4.3) of https://arxiv.org/pdf/1803.10701.pdf */

  REAL8 valAmax = ampWave->data[0];
  REAL8 valdAmax = 0;
  REAL8 tofAmax = matchrange[1];
  UINT4 indAmax;
  if (l == 5 && m == 5) {
    tofAmax = matchrange[3];
  }
  // RC we should find indAmax only for the mode 22, for the other one the
  // attaching point is the same, with the exception of the 55 For the 55 mode
  // we need to find tpeak22 -10M, which in that case is fed as matchrange[3]
  if ((l == 2 && m == 2) || (l == 5 && m == 5)) {
    if (XLALSimFindIndexMaxAmpli(&indAmax, timeVec, ampWave, &valAmax,
                                 tofAmax) == XLAL_FAILURE) {
    ...
    // RC for the 55 mode we also need to find the derivative at the attachment
    // point, since we are not making the attachment at the peak of the mode
    // where the derivative is zero, see Eq.(4.3) of
    ...
    /*For the modes different from 22, this IS NOT the value of the maximum of
     * the amplitude of the mode, but is the value of the amplitude at the peak
     * of the 22 mode*/
    /*For the SEOBNRv4HM model we need to compute also the derivative at the
     * attaching point. This is not zero because we are attaching the HMs not at
     * the paek of the lm mode, but at the peak of the 22 mode*/
  ...
  if (debugSB) {
    printf(
        "Check: The maximum of amplitude is %.16e found at t=%f, index = %d "
        "(out of %d) \n",
  ...
  // chi + A1coeff10[l][m] * eta + A1coeff11[l][m] * eta * chi + A1coeff20[l][m]
  // * eta * eta;
  ...
  /* Eq. (55) of https://dcc.ligo.org/T1600383 for SEOBNRv4 and 22 mode
   * SEOBNRv4HM*/
  /* Eqs. (C2, C4, C6, C8) of https://arxiv.org/pdf/1803.10701.pdf for
   * SEOBNRv4HM*/
  REAL8 ampcf2;
  // ampcf2 = A2coeff00[l][m] + A2coeff01[l][m] * chi + A2coeff10[l][m] * eta +
  // A2coeff11[l][m] * eta * chi + A2coeff20[l][m] * eta * eta + A2coeff21[l][m]
  // * eta * eta * chi;
  ...
  REAL8 dtArescaledtcons =
      (valdAmax - creal(sigmalm0) * valAmax) * exp(-creal(sigmalm0) * tcons) /
      eta;  // valdAmax = 0 - assumes extermum (max) of peak amplitude //RC:
            // valdAmax = 0 for 22 mode
  ...
  REAL8 ampcc2 = (Arescaledtcons * ampcf1 -
                  dtArescaledtcons * cosh(ampcf2) * sinh(ampcf2)) /
                 ampcf1;
  ...
  for (i = 0; i < Nrdwave; i++) {
    ampRD->data[i] =
        eta * exp(creal(sigmalm0) * rdtime->data[i]) *
  ...
  for (i = 0; i < Nrdwave; i++) {
    ...
    phRD->data[i] = phi0 - phasecc1 * log(logargnum / logargden) +
                    cimag(sigmalm0) * rdtime->data[i];
  }
  if (debugSB) {
...
static UNUSED INT4 XLALSimIMREOBTaper(
  ...
  sigma220 = (-cimag(modefreqs->data[0]) - I * creal(modefreqs->data[0])) *
             (mtot * LAL_MTSUN_SI);
   ...
   * SEOBNRv4HM*/
  REAL8 ampcf1;
  // ampcf1 = A1coeff00 + A1coeff01 * chi + A1coeff02 * chi * chi + A1coeff10 *
  // eta + A1coeff11 * eta * chi + A1coeff20 * eta * eta;
  ampcf1 = XLALCalculateRDAmplitudeCoefficient1(l, m, eta, chi);
   ...
   * SEOBNRv4HM*/
  REAL8 ampcf2;
  // ampcf2 = A2coeff00 + A2coeff01 * chi + A2coeff10 * eta + A2coeff11 * eta *
  // chi + A2coeff20 * eta * eta + A2coeff21 * eta * eta * chi;
  ampcf2 = XLALCalculateRDAmplitudeCoefficient2(l, m, eta, chi);
  ...
  REAL8 phasecf1;
  // phasecf1 = P1coeff00 + P1coeff01 * chi + P1coeff02 * chi * chi + P1coeff10
  // * eta + P1coeff11 * eta * chi + P1coeff20 * eta * eta;
  phasecf1 = XLALCalculateRDPhaseCoefficient1(l, m, eta, chi);

  /* Eq. (37) of https://dcc.ligo.org/T1600383 */
  REAL8 phasecf2;
  // phasecf2 = P2coeff00 + P2coeff01 * chi + P2coeff02 * chi * chi + P2coeff10
  // * eta + P2coeff11 * eta * chi + P2coeff20 * eta * eta;
  phasecf2 = XLALCalculateRDPhaseCoefficient2(l, m, eta, chi);
  ...
  REAL8 dtArescaledtcons = (0. - creal(sigma220) * valAmax) *
                           exp(-creal(sigma220) * tcons) /
                           eta;  // 0 - assumes extermum (max) of peak amplitude
  REAL8 ampcc1 = dtArescaledtcons * pow(cosh(ampcf2), 2) / ampcf1;
  REAL8 ampcc2 = (Arescaledtcons * ampcf1 -
                  dtArescaledtcons * cosh(ampcf2) * sinh(ampcf2)) /
                 ampcf1;
  ...
  for (i = 0; i < Nrdwave; i++) {
    ampRD->data[i] =
        ...
        (ampcc1 * tanh(ampcf1 * rdtime->data[i] + ampcf2) + ampcc2);
    //        ampRD->data[i] = valAmax/dpar * exp(creal(sigma220) *
    //        rdtime->data[i]) * (dpar + tanh(dpar*creal(sigma220) *
    //        rdtime->data[i]));
  }
  if (debugSB) {
  ...
  for (i = 0; i < Nrdwave; i++) {
    ...
    logargden = 1. + phasecf2;
    phRD->data[i] = phi0 - phasecc1 * log(logargnum / logargden) +
                    cimag(sigma220) * rdtime->data[i];
    phRD->data[i] =
        phi0 + ((phWave->data[indAmax] - phWave->data[indAmax - 1]) / dtM) *
                   rdtime->data[i];
  }
