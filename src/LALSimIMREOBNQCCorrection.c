 ...
 *
 * \brief More recent versions of the EOB models, such as EOBNRv2 and SEOBNRv1,
 * utilise a non-quasicircular correction (NQC) to bring the peak of the EOB
 * frequency into agreement with that of NR simulations. This file contains the
 * functions used to calculate these NQC corrections, described in DCC document
 * T1100433. The fits to NR peak amplitude, frequency, and their derivatives,
 * are taken from Pan et al. PRD 84 124052 (2011) [arXiv:1106.1021], for
 * EOBNRv2, and from Taracchini et al. PRD 86, 024011 (2012) [arXiv:1202.0790],
 * for SEOBNRv1.
 */
#pragma once
 ...
 * are taken from Pan et al, PRD84, 124052(2011) [arXiv:1106.1021], lines
 * 1-5 of Table III.
 */
 ...
 * are taken from Pan et al, PRD84, 124052(2011) [arXiv:1106.1021], lines 1-5 of
 * Table II.
 */
static inline REAL8 GetNRPeakAmplitude(INT4 l,   /**<< Mode l */
                                       INT4 m,   /**<< Mode m */
  ...
  switch (l) {
    ...
    case 3:
      if (m == 3) {
        return eta * sqrt(1. - 4. * eta) *
               (0.5761 - 0.09638 * eta + 2.715 * eta * eta);
      } else {
        XLAL_ERROR_REAL8(XLAL_EINVAL);
      }
      break;
 ...
 * Table II.
 */
static inline REAL8 GetNRPeakADDot(INT4 l,   /**<< Mode l */
                                   INT4 m,   /**<< Mode m */
                                   REAL8 eta /**<< Symmetric mass ratio */
) {
  switch (l) {
    ...
    case 3:
      if (m == 3) {
        return -0.01 * eta * sqrt(1. - 4. * eta) *
               (0.2518 - 0.8145 * eta + 5.731 * eta * eta);
      } else {
        XLAL_ERROR_REAL8(XLAL_EINVAL);
      }
      break;
 ...
 * Table II.
 */
static inline REAL8 GetNRPeakOmega(INT4 l,   /**<< Mode l */
                                   INT4 m,   /**<< Mode m */
                                   REAL8 eta /**<< Symmetric mass ratio */
) {
  switch (l) {
 ...
 * Table II.
 */
static inline REAL8 GetNRPeakOmegaDot(INT4 l,   /**<< Mode l */
                                      INT4 m,   /**<< Mode m */
                                      REAL8 eta /**<< Symmetric mass ratio */
) {
  switch (l) {
...
UNUSED static int XLALSimIMREOBGetCalibratedNQCCoeffs(
  ...
  /* including coeffs->a3S, coeffs->a4 and coeffs->a5 that are not used in
   * EOBNRv2 */
 ...
 * PRD84, 124052(2011) [arXiv:1106.1021], Eq.(22), and also in the DCC document
 * T1100433. Note that when calling this function, the NQC coefficients should
...
UNUSED static int XLALSimIMREOBNonQCCorrection(
  ...
  printf("b1 = %.16e, b2 = %.16e, b3 = %.16e, b4 =
 ...
 * PRD84, 124052(2011) [arXiv:1106.1021], Eq.(22), and also in the DCC document
 * T1100433. Note that when calling this function, the NQC coefficients should
 * already have been pre-computed. This version is for generic precesing case
 * where the dynamics variable values are given in Catesean coordinates.
 */
UNUSED static int XLALSimIMRSpinEOBNonQCCorrection(
  ...
  p = (values->data[0] * values->data[3] + values->data[1] * values->data[4] +
       values->data[2] * values->data[5]) /
      r;
...
UNUSED static int XLALSimIMREOBCalculateNQCCoefficients(
  ...
  XLAL_CALLGSL(
      /* a stuff */
      qMatrix = gsl_matrix_alloc(3, 3); aCoeff = gsl_vector_alloc(3);
      amps = gsl_vector_alloc(3); perm1 = gsl_permutation_alloc(3);
      /* b stuff */
      pMatrix = gsl_matrix_alloc(2, 2);
      bCoeff = gsl_vector_alloc(2); omegaVec = gsl_vector_alloc(2);
...
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakDeltaT(
  ...
  switch (l) {
    case 2:
      switch (m) {
        case 2:
          /* DeltaT22 defined here is a minus sign different from Eq. (33) of
           * Taracchini et al. */
          if (a <= 0.0) {
            return 2.5;
          } else {
            return (2.5 + 1.77 * a * a * a * a /
                              (0.43655 * 0.43655 * 0.43655 * 0.43655) /
...
/**
 * Peak amplitude predicted by fitting NR results (currently only 2,2
 * available). Unpublished. Used in building SEOBNRv2 tables.
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakAmplitudeV2(
    INT4 UNUSED l, INT4 UNUSED m, REAL8 UNUSED eta, REAL8 UNUSED a) {
  REAL8 chi = a, chi2 = chi * chi, chi3 = chi * chi2;
  REAL8 eta2 = eta * eta;
  REAL8 res;
  if (chi > 0.8) {
    res = eta * (56.28859370276537 * (-0.1858184673895021 + eta) *
                     (0.18616944529501114 + eta) -
                 155.11365222671776 * chi3 * (-0.25025223669804486 + eta) *
                     (0.23614334403451426 + eta) +
                 322.4309641674941 * chi2 * (-0.24986765309607953 + eta) *
                     (0.24802475468124208 + eta) -
                 226.09242469439047 * chi * (-0.24993985462384588 + eta) *
                     (0.2573225045218015 + eta));
  } else {
    res = eta * (1.449934273310975 +
                 3.8867234144877933 * chi * (-0.26967339454732164 + eta) *
                     (-0.15977862405445659 + eta) +
                 2.2705573440821687 * chi2 * (-0.20039719578235954 + eta) *
                     (-0.06130397389190033 + eta) -
                 8.094119513915285 * chi3 * (-0.2598144292071539 + eta) *
                     (-0.010564809220517786 + eta) +
...
/**
 * Combine two spin-dependent fits of NR input values in a quardatic-in-eta
 * polynomial with the linear-in-eta coefficient A1 being provided by a
 * polyinamial-in-spin global fit to NR in the whole eta-chi plane. See Eqs.
 * (20)-(21) in https://dcc.ligo.org/T1600383
 */
static inline REAL8 CombineTPLEQMFits(REAL8 eta, REAL8 A1, REAL8 fEQ,
                                      REAL8 fTPL) {
  REAL8 A0, A2;
  REAL8 eta2 = eta * eta;
  // Impose that TPL and equal-mass limit are exactly recovered
  A0 = -0.00099601593625498 * A1 - 0.00001600025600409607 * fEQ +
       1.000016000256004 * fTPL;
  A2 = -3.984063745019967 * A1 + 16.00025600409607 * fEQ -
       16.0002560041612 * fTPL;
  // Final formula
 ...
 * The coefficients for the mode (2,2) are in Eq.(A1) of
 * https://journals.aps.org/prd/pdf/10.1103/PhysRevD.95.044028 The coefficients
 * for the modes (2,1), (3,3), (4,4), (5,5) are in Eqs. (B7-B10) of
 * https://arxiv.org/pdf/1803.10701.pdf
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakAmplitudeV4(
    INT4 modeL, INT4 modeM, REAL8 m1, REAL8 m2, REAL8 chiS, REAL8 chiA) {
  REAL8 eta = (m1 * m2) / ((m1 + m2) * (m1 + m2));
  REAL8 dM = sqrt(1. - 4. * eta);
  REAL8 tempm1;
  if (m1 < m2) {
    // RC: The fits for the HMs are done under the assumption m1>m2, so if m2>m1
    // we just swap the two bodies
    tempm1 = m1;
    m1 = m2;
    m2 = tempm1;
    chiA = -chiA;
  ...
  switch (modeL) {
    case 2:
      switch (modeM) {
        case 2:
          // TPL fit
          fTPL = 1.4528573105413543 + 0.16613449160880395 * chi +
                 0.027355646661735258 * chi2 - 0.020072844926136438 * chi3;
          // Equal-mass fit
          fEQ = 1.577457498227 - 0.0076949474494639085 * chi +
                0.02188705616693344 * chi2 + 0.023268366492696667 * chi3;
        ...
        case 1:
          ...
          res = -((0.29256703361640224 - 0.19710255145276584 * eta) * eta *
                      chi21 +
                  dM * eta *
                      (-0.42817941710649793 + 0.11378918021042442 * eta -
                       0.7736772957051212 * eta2 +
                       chi21 * chi21 *
                           (0.047004057952214004 - eta * 0.09326128322462478)) +
                  dM * eta * chi21 *
                      (-0.010195081244587765 +
                       0.016876911550777807 * chi21 * chi21));

          break;

        default:
          XLALPrintError(
              "XLAL Error - %s: (%d,%d) mode. At present only fits for the "
              "(2,2), (2,1), (3,3), (4,4) and (5,5) mode are available.\n",
    ...
    case 3:
      switch (modeM) {
        case 3:
          res = (0.10109183988848384 * eta - 0.4704095462146807 * eta2 +
                 1.0735457779890183 * eta2 * eta) *
                    chi33 +
                dM * (0.5636580081367962 * eta - 0.054609013952480856 * eta2 +
                      2.3093699480319234 * eta2 * eta +
                      chi33 * chi33 *
                          (0.029812986680919126 * eta -
                           0.09688097244145283 * eta2));
          break;
    ...
    case 4:
      switch (modeM) {
        case 4:
          res = eta * (0.2646580063832686 + 0.067584186955327 * chi44 +
                       0.02925102905737779 * chi44 * chi44) +
                eta2 * (-0.5658246076387973 - 0.8667455348964268 * chi44 +
                        0.005234192027729502 * chi44 * chi44) +
                eta * eta2 *
                    (-2.5008294352355405 + 6.880772754797872 * chi44 -
    ...
    case 5:
      switch (modeM) {
        case 5:
          ...
          res = 0.128621 * dM * eta - 0.474201 * dM * eta * eta +
                1.0833 * dM * eta * eta * eta + 0.0322784 * eta * chi33 -
    ...
    default:
      XLALPrintError(
          "XLAL Error - %s: (%d,%d) mode. At present only fits for the (2,2), "
          "(2,1), (3,3), (4,4) and (5,5) mode are available.\n",
          __func__, modeL, modeM);
      XLAL_ERROR(XLAL_EINVAL);
      break;
  }
 ...
 * attachment is done at the peak of the (2,2) mode see Eq.(2.7) in
 * https://journals.aps.org/prd/pdf/10.1103/PhysRevD.95.044028 The coefficients
 * for the modes (2,1), (3,3), (4,4) and (5,5) are in Eqs.(B11-B14) of
 * https://arxiv.org/pdf/1803.10701.pdf
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakADotV4(
    INT4 modeL, INT4 modeM, REAL8 UNUSED m1, REAL8 UNUSED m2, REAL8 UNUSED chiS,
    REAL8 UNUSED chiA)

{
  ...
  switch (modeL) {
      ...
      switch (modeM) {
        ...
        case 1:
          res = dM * eta * (0.007147528020812309 - eta * 0.035644027582499495) +
                dM * eta * chi21 *
                    (-0.0087785131749995 + eta * 0.03054672006241107) +
                eta * 0.00801714459112299 *
                    fabs(-dM * (0.7875612917853588 + eta * 1.161274164728927 +
    ...
    case 3:
      switch (modeM) {
        case 3:
          res = dM * eta * (-0.00309943555972098 + eta * 0.010076527264663805) *
                    chi33 * chi33 +

                eta * 0.0016309606446766923 *
                    sqrt(dM2 * (8.811660714437027 + 104.47752236009688 * eta) +
                         dM * chi33 *
                             (-5.352043503655119 + eta * 49.68621807460999) +
                         chi33 * chi33);
          break;
        default:
          XLALPrintError(
    ...
    case 4:
      switch (modeM) {
        case 4:
          res =
              eta * (0.004347588211099233 - 0.0014612210699052148 * chi44 -
                     0.002428047910361957 * chi44 * chi44) +
              eta2 * (0.023320670701084355 - 0.02240684127113227 * chi44 +
        ...
        case 5:
          res = eta *
                (dM * (-0.008389798844109389 + 0.04678354680410954 * eta) +
                 dM * chi33 *
                     (-0.0013605616383929452 + 0.004302712487297126 * eta) +
                 dM * chi33 * chi33 *
                     (-0.0011412109287400596 + 0.0018590391891716925 * eta) +

                 0.0002944221308683548 *
                     fabs(dM * (37.11125499129578 - 157.79906814398277 * eta) +
                          chi33));
          break;
        default:
          XLALPrintError(
...
/**
 * Peak amplitude curvature predicted by fitting NR results (currently only 2,2
 * available). Unpublished. Used in building SEOBNRv2 tables.
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakADDotV2(INT4 UNUSED l,
                                                             INT4 UNUSED m,
                                                             REAL8 UNUSED eta,
                                                             REAL8 UNUSED a) {
  REAL8 chi = a;
  REAL8 res;
  if (chi > 0.8) {
    res = eta * (0.04818855743392375 * chi * (-0.15271017198256195 + eta) *
                     (0.2154794478856639 + eta) -
                 0.038199344157345716 * (-0.06621184971565616 + eta) *
                     (0.31317077454081577 + eta));
  } else {
    res =
        eta *
        (0.010916757595083287 * (-1.0608229327701018 + eta) *
             (0.19667724848989968 + eta) -
         0.007331284524315633 * chi * (-0.753628708015681 + eta) *
             (0.341046049832081 + eta) +
         chi * chi * (0.0006958660609341137 - 0.01113385697494582 * eta * eta) +
         chi * chi * chi *
             (-0.00029607425270115136 + 0.004737188043218422 * eta * eta));
  }
  return res;
}
 ...
 * The coefficients for the (2,2) are in Eq.(A3-A4) of
 * https://journals.aps.org/prd/pdf/10.1103/PhysRevD.95.044028 The coefficients
 * for the modes (2,1), (3,3), (4,4) and (5,5) are in Eqs.(B15-B18) of
 * https://arxiv.org/pdf/1803.10701.pdf
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakADDotV4(
    INT4 modeL, INT4 modeM, REAL8 m1, REAL8 m2, REAL8 chiS, REAL8 chiA) {
  REAL8 eta = (m1 * m2) / ((m1 + m2) * (m1 + m2));
  REAL8 dM = sqrt(1. - 4. * eta);
  REAL8 tempm1;
  ...
  switch (modeL) {
    case 2:
      switch (modeM) {
        case 2:
          // TPL fit
          fTPL = 0.002395610769995033 * chiMinus1 -
        ...
        case 1:
          ...
          res = eta * dM * 0.00037132201959950333 -
                fabs(dM * eta *
                         (-0.0003650874948532221 - eta * 0.003054168419880019) +
                     dM * eta * chi21 * chi21 *
                         (-0.0006306232037821514 - eta * 0.000868047918883389 +
        ...
        default:
          XLALPrintError(
              "XLAL Error - %s: At present only fits for the (2,2), (2,1), "
        ...
        case 3:
          res = dM * eta *
                ...
                0.00015623760412359145 * eta *
                    fabs(dM * (4.676662024170895 + 79.20189790272218 * eta -
        ...
        case 5:
          res = eta *
                (dM * (0.00012727220842255978 + 0.0003211670856771251 * eta) +
                 dM * chi33 *
                     (-0.00006621677859895541 + 0.000328855327605536 * eta) +
                 chi33 * chi33 *
...
/**
 * Peak frequency predicted by fitting NR results (currently only 2,2
 * available). Unpublished. Used in building SEOBNRv2 tables.
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmegaV2(INT4 UNUSED l,
                                                             INT4 UNUSED m,
                                                             REAL8 UNUSED eta,
                                                             REAL8 a) {
  REAL8 chi = a;
  ...
  REAL8 res;
  if (eta > 50. / 51. / 51.) {
    res = 0.43747541927878864 +
          (-0.10933208665273314 - 0.007325831113333813 * chi) *
    ...
    res = 1.5609526077704716 - 122.25721149839733 * eta +
          3586.2192688666914 * eta2 - 13869.506144441548 * eta3 +
          (-0.25 + eta) *
              (1651.5823693445805 * (-0.019223375977400495 + eta) *
                   (-0.01922337527211892 + eta) +
               66.87492814925524 * chi *
                   (0.0003695381704106058 - 0.03844675124951941 * eta + eta2)) *
              log(5600.67382718678 - 5555.824895398546 * chi) +
          (-1412.8186461833657 + 67.66455403259023 * chi) * (-0.001 + eta) *
              (0.0003695381704106056 - 0.038446751249519406 * eta + eta2) *
              log(0.5680439481719505 - 0.36813967358200156 * chi) +
          0.012328326527732041 *
              log(4.500844771420863 - 9.681916048928946 * eta +
                  chi * (-4.254886879579986 + 11.513558950322647 * eta)) +
          0.0008260634258180991 * chi *
              log(4.500844771420863 - 9.681916048928946 * eta +
                  chi * (-4.254886879579986 + 11.513558950322647 * eta)) -
          12.6575493872956 * eta *
              log(4.500844771420863 - 9.681916048928946 * eta +
                  chi * (-4.254886879579986 + 11.513558950322647 * eta)) -
          0.8481231078533651 * chi * eta *
              log(4.500844771420863 - 9.681916048928946 * eta +
                  chi * (-4.254886879579986 + 11.513558950322647 * eta)) +
          329.2228595635586 * eta2 *
              log(4.500844771420863 - 9.681916048928946 * eta +
                  chi * (-4.254886879579986 + 11.513558950322647 * eta)) +
          22.05968203526603 * chi * eta2 *
              log(4.500844771420863 - 9.681916048928946 * eta +
                  chi * (-4.254886879579986 + 11.513558950322647 * eta));
  }
  return res;
 ...
 * The coefficients for the (2,2) are in Eq.(A6) of
 * https://journals.aps.org/prd/pdf/10.1103/PhysRevD.95.044028 The coefficients
 * for the modes (2,1), (3,3), (4,4) and (5,5) are in Eqs.(B19-B22) of
 * https://arxiv.org/pdf/1803.10701.pdf
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmegaV4(INT4 modeL,
                                                             INT4 modeM,
                                                             REAL8 UNUSED eta,
                                                             REAL8 a) {
  ...
  switch (modeL) {
    case 2:
      switch (modeM) {
        case 2:
          ...
          break;
        case 1:
          res = (0.1743194440996283 + eta * 0.1938944514123048 +
                 0.1670063050527942 * eta2 + 0.053508705425291826 * chi -
    ...
    case 3:
      switch (modeM) {
        case 3:
          res = 0.3973947703114506 + 0.16419332207671075 * chi +
                0.1635531186118689 * chi * chi +
                0.06140164491786984 * chi * chi * chi +
                eta * (0.6995063984915486 - 0.3626744855912085 * chi -
                       0.9775469868881651 * chi * chi) +
                eta2 * (-0.3455328417046369 + 0.31952307610699876 * chi +
    ...
    case 4:
      switch (modeM) {
        case 4:
          res = 0.5389359134370971 + 0.16635177426821202 * chi +
                0.2075386047689103 * chi * chi +
                0.15268115749910835 * chi * chi * chi +
                eta * (0.7617423831337586 + 0.009587856087825369 * chi -
                       1.302303785053009 * chi * chi -
                       0.5562751887042064 * chi * chi * chi) +
                eta2 * (0.9675153069365782 - 0.22059322127958586 * chi +
    ...
    case 5:
      switch (modeM) {
        case 5:
          res = 0.6437545281817488 + 0.22315530037902315 * chi +
                0.2956893357624277 * chi * chi +
                0.17327819169083758 * chi * chi * chi +
                eta * (-0.47017798518175785 - 0.3929010618358481 * chi -
                       2.2653368626130654 * chi * chi -
                       0.5512998466154311 * chi * chi * chi) +
                eta2 * (2.311483807604238 + 0.8829339243493562 * chi +
...
/**
 * Peak frequency slope predicted by fitting NR results (currently only 2,2
 * available). Unpublished. Used in building SEOBNRv2 tables.
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmegaDotV2(
    INT4 UNUSED l, INT4 UNUSED m, REAL8 UNUSED eta, REAL8 UNUSED a) {
  REAL8 chi = a;
  REAL8 res;
  if (chi > 0.8) {
    res = -0.10069512275335238 * (-0.46107388514323044 + eta) *
              (0.2832795481380979 + eta) +
          0.2614619716504706 * chi * (-0.24838163750494138 + eta) *
              (0.320112993649413 + eta) +
          chi * chi * (0.010000160002560042 - 0.16000256004096067 * eta * eta);
  } else {
    res = -0.07086074186161867 * chi * (-0.26367236731979804 + eta) *
              (-0.0010019969893089581 + eta) +
          0.2893863668183948 * (-0.16845695144529893 + eta) *
              (0.23032241797163952 + eta) +
          (0.004086861548547749 - 0.06538978477676398 * eta * eta +
           chi * (0.0006334026884930817 - 0.010134443015889307 * eta * eta)) *
 ...
 * The coefficients for the (2,2) are in Eq.(A10) of
 * https://journals.aps.org/prd/pdf/10.1103/PhysRevD.95.044028 The coefficients
 * for the modes (2,1), (3,3), (4,4) and (5,5) are in Eqs.(B23-B26) of
 * https://arxiv.org/pdf/1803.10701.pdf
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmegaDotV4(
    INT4 modeL, INT4 modeM, REAL8 UNUSED eta, REAL8 UNUSED a) {
  REAL8 chi = a, eta2 = eta * eta;
  REAL8 res;
  REAL8 fTPL, fEQ, A1, e0, e1;
  switch (modeL) {
    case 2:
      switch (modeM) {
        ...
        case 1:
          res =
              (0.0070987396362959514 + eta * 0.024816844694685373 -
               eta2 * 0.050428973182277494 + eta * eta2 * 0.03442040062259341 -
               chi * 0.0017751850002442097 + eta * chi * 0.004244058872768811 -
    ...
    case 3:
      switch (modeM) {
        case 3:
          res = 0.010337157192240338 - 0.0053067782526697764 * chi * chi -
                0.005087932726777773 * chi * chi * chi +
                eta * (0.027735564986787684 + 0.018864151181629343 * chi +
    ...
    case 4:
      switch (modeM) {
        case 4:
          res = 0.013997911323773867 - 0.0051178205260273574 * chi -
                0.0073874256262988 * chi * chi +
                eta * (0.0528489379269367 + 0.01632304766334543 * chi +
    ...
    case 5:
      switch (modeM) {
        case 5:
          res = 0.01763430670755021 - 0.00024925743340389135 * chi -
                0.009240404217656968 * chi * chi -
                0.007907831334704586 * chi * chi * chi +
                eta * (-0.1366002854361568 + 0.0561378177186783 * chi +
                       0.16406275673019852 * chi * chi +
                       0.07736232247880881 * chi * chi * chi) +
                eta2 * (0.9875890632901151 - 0.31392112794887855 * chi -
...
/**
 * Peak amplitude predicted by fitting NR results (currently only 2,2
 * available). Tables IV and V and Eq. 42 of Taracchini et al. PRD 86, 024011
 * (2012) [arXiv:1202.0790].
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakAmplitude(INT4 UNUSED l,
                                                               INT4 UNUSED m,
                                                               REAL8 UNUSED eta,
                                                               REAL8 UNUSED a) {
 ...
 * (2012) [arXiv:1202.0790].
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakADDot(INT4 UNUSED l,
                                                           INT4 UNUSED m,
                                                           REAL8 UNUSED eta,
                                                           REAL8 UNUSED a) {
  /* Fit for HOMs missing */
  return eta *
         (-0.0024971911410897156 +
          (-0.006128515435641139 + 0.01732656 * a / (2.0 - 4.0 * eta)) * eta);
}
 ...
 * (2012) [arXiv:1202.0790].
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmega(INT4 UNUSED l,
                                                           INT4 UNUSED m,
                                                           REAL8 UNUSED eta,
                                                           REAL8 a) {
  /* Fit for HOMs missing */
  return 0.27581190323955274 + 0.19347381066059993 * eta -
         ...
         eta * eta *
             (1.78832 * (0.2690779744133912 + a / (2.0 - 4.0 * eta)) *
 ...
 * (2012) [arXiv:1202.0790].
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmegaDot(INT4 UNUSED l,
                                                              INT4 UNUSED m,
                                                              REAL8 UNUSED eta,
                                                              REAL8 UNUSED a) {
  /* Fit for HOMs missing */
  return 0.006075014646800278 + 0.012040017219351778 * eta +
         (0.0007353536801336875 +
          0.0015592659912461832 * a / (1.0 - 2.0 * eta)) *
             log(1.0 - a / (1.0 - 2.0 * eta)) +
         eta * eta *
             (0.03575969677378844 +
              (-0.011765658882139 -
               0.02494825585993893 * a / (1.0 - 2.0 * eta)) *
                  log(1.0 - a / (1.0 - 2.0 * eta)));
}
...
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakDeltaTv2(
  ...
  REAL8 eta = m1 * m2 / ((m1 + m2) * (m1 + m2));
  chi = (chi1 + chi2) / 2. +
        (chi1 - chi2) / 2. * ((m1 - m2) / (m1 + m2)) / (1 - 2. * eta);

  chichi = chi * chi;
  if (chi > 0.8) {
    return (0.75 * eta * chi + sqrt(1. - 4. * eta)) * (57.1755 - 48.0564 * chi);
  } else if (chi > 0.0) {
    return (0.75 * eta * chi + sqrt(1. - 4. * eta)) *
           (2.5 + 10. * chichi + 24. * chichi * chichi);
  } else {
...
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakDeltaTv4(
  ...
  res = coeff00 + coeff01 * chi + coeff02 * chiTo2 + coeff03 * chiTo3 +
        ...
        coeff31 * eta3 * chi + coeff32 * eta3 * chiTo2 +
        coeff33 * eta3 * chiTo3;

  // RC: for the 55 mode the attachment is done at tpeak22 -10M, note that since
  // here deltat22 is defined as -deltat22 with respect to SEOBNRv4 paper, here
  // I need to add 10 M
  if ((l == 5) && (m == 5)) {
    res = res + 10;
  }
...
/**
 * Peak frequency predicted by fitting NR results (currently only 2,2
 * available). Take from unpublished SEOBNRv2 results.
 */
UNUSED static inline REAL8 XLALSimIMREOBGetNRSpinPeakOmegav2(INT4 UNUSED l,
                                                             INT4 UNUSED m,
                                                             REAL8 UNUSED eta,
  ...
  if (eta > 50. / 51. / 51.) {
    ...
    return 1.5609526077704716 - 122.25721149839733 * eta +
           ...
           (eta - 0.25) *
               (1651.5823693445805 * (-0.01922337588094282 + eta) *
                    (-0.01922337536857659 + eta) +
                66.87492814925524 * chi *
                    (0.0003695381704106058 - 0.03844675124951941 * eta +
                     eta2)) *
               log(5600.67382718678 - 5555.824895398546 * chi) +
...
UNUSED static int XLALSimIMRSpinEOBCalculateNQCCoefficients(
  ...
  switch (SpinAlignedEOBversion) {
    case 1:
      ...
      break;
    case 2:
      // if ( XLALSimIMRGetEOBCalibratedSpinNQCv2( coeffs, l, m, eta, a ) ==
      // XLAL_FAILURE )
 ...
 * coefficients are found in the DCC document T1100433.
 * In brief, this function populates and solves the linear equations
 * Eq. 18 (for amplitude) and Eq. 19 (for phase) of
 * https://dcc.ligo.org/T1100433
 */
UNUSED static int XLALSimIMRSpinEOBCalculateNQCCoefficientsV4(
  ...
  nra = fabs(
      XLALSimIMREOBGetNRSpinPeakAmplitudeV4(modeL, modeM, m1, m2, chiS, chiA));

  nraDot = XLALSimIMREOBGetNRSpinPeakADotV4(modeL, modeM, m1, m2, chiS, chiA);
  // RC: In SEOBNRv4 nraDot is zero because the NQC are defining the peak of the
  // 22 mode
  ...
  //    m2)/(1. - 2.*eta));
  if (XLAL_IS_REAL8_FAIL_NAN(nra) || XLAL_IS_REAL8_FAIL_NAN(nraDot) ||
      XLAL_IS_REAL8_FAIL_NAN(nraDDot)) {
    XLALDestroyREAL8Vector(timeVec);
    XLALDestroyREAL8Vector(q3);
    XLALDestroyREAL8Vector(q4);
    XLALDestroyREAL8Vector(q5);
  ...
  nromega = XLALSimIMREOBGetNRSpinPeakOmegaV4(
      modeL, modeM, eta, chiS + chiA * (m1 - m2) / (m1 + m2) / (1. - 2. * eta));
